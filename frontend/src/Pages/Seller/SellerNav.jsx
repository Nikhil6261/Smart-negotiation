  ...
  return (
    <nav className="border-b border-slate-200 bg-blue/80 backdrop-blur px-8 py-4 flex justify-between items-center sticky top-0 z-50">
      ...
      <div
        ...
        onClick={() => navigate("/sellerdash")}
      >
        <div className="bg-indigo-600 p-2 rounded-xl text-white shadow-lg shadow-indigo-600/10">
          <ShoppingBag size={20} />
        </div>
        <div>
          <span className="font-bold tracking-tight text-lg block">
            Negotio<span className="text-indigo-600">.ai</span>
          </span>
          <span className="text-[10px] uppercase tracking-wider block text-slate-400 font-bold">
            Seller Portal
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          {/* Dropdown Panel */}
          <div
            className={`absolute right-0 mt-2 w-60 bg-white border border-slate-200 rounded-2xl shadow-xl p-2 z-50 origin-top-right transform transition-all duration-300 ease-out
            ${
              isProfileOpen
